...
### ❌ No Direct Equivalent (Accepted Limitations)
...
**Testing Strategy:**
1. Test rm blocking: `rm file.txt` → should block
2. Test git add blocking: `git add .` → should block
3. Test kubectl blocking: `kubectl delete pod foo` → should block
4. Test terraform blocking: `terraform apply` → should block
5. Test file size with 600-line file → should block
6. Test CLAUDE.md write blocking → should block
7. Test formatting after YAML edit → should format
...
**Testing Strategy:**
1. Trigger session idle → verify notification
2. Trigger permission request → verify notification
...
**Test Scenarios:**
- Plugin throws exception → verify user sees actionable error
- Format script missing → verify graceful degradation
- Flag file permissions issue → verify error message
- Network failure (ntfy) → verify silent fallback
...
> **Secret Management via Home Manager:** Inject the OpenAI key declaratively—never hard-code it into `opencode.json`. Use the Home-Manager secret management flow you already rely on (e.g. `sops-nix`, `agenix`, or `home.file` pointing at an encrypted path).
...
### Critical (Must Have)
- ✅ `rm` commands blocked 100% of time
- ✅ Dangerous `git add` patterns blocked 100% of time
- ✅ Files >500 lines blocked for main agent
- ✅ CLAUDE.md writes blocked 100% of time

### Important (Should Have)
- ✅ kubectl/terraform operations blocked
- ✅ YAML/Terraform files auto-formatted
- ✅ Notifications delivered for key events
- ✅ Plugin execution time <100ms average

### Nice to Have (Could Have)
- ✅ Sub-agent file size limits working
- ✅ Grep enforcement suggesting ripgrep
- ✅ Custom skill activation commands
...
### Update Triggers
- OpenCode releases new hook types → assess migration opportunities
- New safety requirements → extend `global-safety.ts`
- Performance issues → optimize or split plugins
