...
- One worktree per repo; a session pinned to repo A can't worktree into B (derivation follows the A cd-intent / pinned cwd) — start a fresh session in B's clone. For cross-repo *research* (no writes), read `origin` directly via `gh api repos/<org>/<repo>/contents/…` / `gh search code` instead of switching worktrees. (bebc56f7)
