...
- `EnterWorktree(name:)` — idempotent adopt-or-create; clones `--bare` into `~/wt/<repo>/.bare` (override base `$CLAUDE_WORKTREE_BASE`), fetches origin, bases the branch on `origin/main` (so clone staleness can't reach the worktree). In work repos prefix `name` with the ticket key; announce the path and run baseline tests. Never pass `path:` (builtin validates against the *current clone's* worktree list and rejects); never hand-roll `git worktree add`.
...
- The `read_clone_warn` hook (git-worktree-hooks) warns when you Read/Grep/Glob a clone that already has a `~/wt` worktree — a cue to switch (or to confirm you deliberately want the clone's local state), never a block. (cf94a602)
